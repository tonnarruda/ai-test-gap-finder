...
func TestBuildPrompt_SingleFunc(t *testing.T) {
	...
	assert.Contains(t, prompt, "Você é um especialista em testes de software")
	assert.Contains(t, prompt, "Tarefas:")
	assert.Contains(t, prompt, "Formato da resposta:")
	assert.Contains(t, prompt, "1️⃣ Stack identificada")
	assert.Contains(t, prompt, "2️⃣ Cenários de teste recomendados")
	assert.Contains(t, prompt, "3️⃣ Exemplos de testes automatizados")
	assert.Contains(t, prompt, "Arquivo: user.go")
	assert.Contains(t, prompt, "Função/Método: ValidateLogin")
...
func TestBuildPrompt_EmptyBranches(t *testing.T) {
	...
	assert.Contains(t, prompt, "Arquivo: a.go")
	assert.Contains(t, prompt, "Função/Método: Foo")
	assert.Contains(t, prompt, "Cenários de teste recomendados")
}
